...
#
# QA environment instance layer for the resolved service account.
# Basename resolves to "000" per the canonical layer idiom (spec 02 Section 1.3).
# This layer sits between the qa environment and the per-unit service directories.
#
# Instance numbering convention:
#   "000" is the first (and only) instance of the qa environment in this account.
#   Additional environment instances (e.g. "001") would represent isolated parallel
#   qa runs. The "000" sentinel keeps the namespace consistent with the service
...
# from common/domains.json keyed by env-class (false for "qa", true for "prod");
...
#   The qa subtree (<service-account>/qa/000) is reconciled against the UNION
#   of both prod subtrees:
#     prod service subtree:  <service-account>/prod/000
#     DNS-owner subtree:     <dns-owner-account>/prod/000
#   Parity holds when every qa unit name present in the prod union pins the
#   IDENTICAL ?ref= module-source pin, and no qa unit sources a module/version
...
#   These three units are NOT required to have a qa counterpart.
#   Only permitted differences between qa and prod:
...
#   Any SHARED unit with a divergent ?ref= pin or any qa unit whose
...
# Local apply only (D33): cycle runs under AWS_PROFILE=qa; no GitHub Actions
# workflow and no OIDC role are used for the qa integration gate.
...
locals {
  ...
  #   - "*_role" (qa_role|...|dns_owner_role) = a per-account bootstrap role (bootstrap/)
  ...
  # (false for "qa", true for "prod") and exposed via include.root.locals (D31).
