# kms-key basic example -- terraform-docs reference
...
| description | Description of the KMS key. | string | "Customer managed KMS key - basic example" | no |
| deletion_window_in_days | Waiting period in days before key deletion. | number | 30 | no |
...
| multi_region | Whether the key is a multi-region key. | bool | false | no |
| policy_json | A valid JSON key resource policy. Null uses the default KMS key policy. | string | null | no |
