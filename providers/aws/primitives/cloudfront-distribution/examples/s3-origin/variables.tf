...
variable "bucket_name_suffix" {
  type        = string
  description = "Unique suffix appended to the throwaway S3 origin and CloudFront-log bucket names. Injected at test time via TF_VAR_bucket_name_suffix. The statically-resolvable offline default lets the misconfiguration scanner (trivy) resolve the concrete bucket names and prove that S3 server access logging is ENABLED on the self-logging CloudFront-log destination bucket (a fully opaque name defeats the logging-destination dataflow and re-triggers AWS-0089/AWS-0132)."
