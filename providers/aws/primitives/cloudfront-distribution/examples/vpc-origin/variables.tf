variable "vpc_origin_alb_arn" {
  type        = string
  description = "ARN of the internal Application Load Balancer (or NLB/EC2) CloudFront reaches privately via the VPC origin. On the apply path this must be a real, Active internal ALB ARN (CreateVpcOrigin validates the ARN). The statically-resolvable offline default lets `terraform validate` and the negative plan tests exercise the VPC-origin HCL path without standing up an ALB. Negative tests inject an empty string to exercise the module's origin.vpc_origin_arn validation."
  default     = "arn:aws:elasticloadbalancing:us-east-1:123456789012:loadbalancer/app/offline-internal-alb/0000000000000000"
}

variable "vpc_origin_domain" {
  type        = string
  description = "Origin domain_name set on the distribution origin. For an https-only VPC origin this must be a name the origin's TLS certificate covers (CloudFront validates the origin certificate against domain_name via SNI). It is used for the Host header / SNI only -- routing is via the VPC origin (vpc_origin_alb_arn), so a name that resolves to this distribution does not loop. Defaults to a placeholder for offline validation."
  default     = "collector-000.example.com"
}
