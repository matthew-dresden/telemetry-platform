...
| <a name="input_autoscaling"></a> [autoscaling](#input\_autoscaling) | (Optional) Auto Scaling configuration. Required when enable\_autoscaling is true. max\_capacity must be >= min\_capacity. | <pre>object({<br/>    min_capacity       = number<br/>    max_capacity       = number<br/>    cpu_target_percent = optional(number, 60)<br/>    scale_in_cooldown  = optional(number, 300)<br/>    scale_out_cooldown = optional(number, 60)<br/>  })</pre> | `null` | no |
