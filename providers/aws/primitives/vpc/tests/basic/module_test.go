...
import (
	...
	"github.com/matthew-dresden/telemetry-platform/providers/aws/primitives/vpc/tests/helpers"
...
func TestVPCCreation(t *testing.T) {
	...
	for _, example := range examples {
		t.Run(example, func(t *testing.T) {
			...
			// Verify VPC resource exists - resource is in module.example
			helpers.AssertStateContains(t, ctx.Terraform, "module.example.aws_vpc.vpc")
...
func TestVPCResourceCounts(t *testing.T) {
	...
	for _, example := range examples {
		t.Run(example, func(t *testing.T) {
			...
			// Verify exactly one VPC is created - resource is in module.example
			helpers.AssertResourceCountExact(t, ctx.Terraform, "aws_vpc", 1)

			// Verify flow logs are created (enabled in both examples) - resource is in module.example
			helpers.AssertResourceCountExact(t, ctx.Terraform, "aws_flow_log", 1)
...
func TestAllAssertionTypes(t *testing.T) {
	...
	for _, example := range examples {
		t.Run(example, func(t *testing.T) {
			...
			// Resource Assertions - resources are in module.example
			helpers.AssertStateContains(t, ctx.Terraform, "module.example.aws_vpc.vpc")
			helpers.AssertResourceCountExact(t, ctx.Terraform, "aws_vpc", 1)
			helpers.AssertResourceCountExact(t, ctx.Terraform, "aws_subnet", 0)
			helpers.AssertResourceCountExact(t, ctx.Terraform, "aws_flow_log", 1)
