...
# Fixture: Internal ALB (the alb-listener module needs an existing ALB ARN).
...
locals {
  # When load_balancer_arn_override is set, pass it directly so validation error tests can supply invalid ARNs.
  effective_load_balancer_arn = var.load_balancer_arn_override != null ? var.load_balancer_arn_override : aws_lb.fixture.arn
}

module "example" {
  source = "../../"

  load_balancer_arn = local.effective_load_balancer_arn
