...
# Header the sweep lane must carry while its cron schedule is commented out
# (it assumes an AWS OIDC role that does not exist yet).
...
@pytest.mark.unit
def test_sweep_workflow_schedule_trigger_is_disabled_not_deleted(
    sweep_yaml: dict, sweep_text: str
) -> None:
    """terratest-sweep.yml is DISABLED: its daily schedule must be commented out, not deleted.

    The sweep DELETES AWS resources through an OIDC role that does not exist yet, so a
    live cron would fail every night. The schedule (AC-1, spec 4.9) is preserved verbatim
    in comment form so re-enabling it is a pure uncomment; only the manual
    workflow_dispatch entry point stays live.
    """
    assert DISABLED_HEADER in sweep_text, (
        f"terratest-sweep.yml must carry the header {DISABLED_HEADER!r} explaining why the "
        "sweep is disabled."
    )
    on_triggers = sweep_yaml.get(True) or sweep_yaml.get("on") or {}
    assert "schedule" not in on_triggers, (
        "terratest-sweep.yml must NOT declare a live 'schedule' trigger while the sweep is "
        "disabled -- it needs AWS credentials/OIDC and performs account-wide deletes."
    )
    assert "#   - cron:" in sweep_text, (
        "terratest-sweep.yml must preserve its daily cron schedule in commented-out form "
        "(AC-1, spec 4.9) so re-enabling it is a pure uncomment."
    )
