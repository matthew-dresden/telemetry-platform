...
# Header both terragrunt lanes must carry while their automatic triggers are
# commented out (they assume AWS OIDC roles that do not exist yet).
DISABLED_HEADER = "# DISABLED: requires AWS credentials/OIDC; see tracking issue."
...
@pytest.mark.unit
def test_pr_workflow_pull_request_trigger_is_disabled_not_deleted(
    pr_workflow_yaml: dict, pr_workflow_text: str
) -> None:
    """terragrunt-pr.yml is DISABLED: its pull_request trigger must be commented out, not deleted.

    The lane assumes an AWS OIDC plan role that does not exist yet, so leaving the
    pull_request trigger live would make every PR red. The trigger is preserved
    verbatim in comment form so re-enabling it is a pure uncomment; this test locks
    that contract in both directions (no live auto-trigger, no lost declaration).
    ...
    """
    assert DISABLED_HEADER in pr_workflow_text, (
        f"terragrunt-pr.yml must carry the header {DISABLED_HEADER!r} explaining why the "
        "lane is disabled."
    )
    ...
    assert "pull_request" not in on_triggers, (
        "terragrunt-pr.yml must NOT declare a live 'pull_request' trigger while the lane is "
        "disabled -- it needs AWS credentials/OIDC and would fail every PR."
    )
    assert "workflow_dispatch" in on_triggers, (
        "terragrunt-pr.yml must keep a manual 'workflow_dispatch' trigger so the workflow "
        "stays a valid GitHub Actions file while its automatic trigger is commented out."
    )
    assert "#   pull_request:" in pr_workflow_text, (
        "terragrunt-pr.yml must preserve its 'pull_request' trigger in commented-out form "
        "(AC-1, E7-F2-S1-T1) so re-enabling it is a pure uncomment."
    )
...
@pytest.mark.unit
def test_apply_workflow_push_trigger_is_disabled_not_deleted(
    apply_workflow_yaml: dict, apply_workflow_text: str
) -> None:
    """terragrunt-apply.yml is DISABLED: its push-to-main trigger must be commented out.

    This lane applies to PROD via an AWS OIDC role that does not exist yet, so an
    automatic push-to-main trigger would fail (and, once credentials exist, would
    deploy unreviewed). The trigger is preserved verbatim in comment form so
    re-enabling it is a pure uncomment; only the manual workflow_dispatch entry
    point stays live.
    ...
    are checked so the test is robust to both quoted and unquoted forms.
    """
    assert DISABLED_HEADER in apply_workflow_text, (
        f"terragrunt-apply.yml must carry the header {DISABLED_HEADER!r} explaining why the "
        "lane is disabled."
    )
    on_triggers = apply_workflow_yaml.get(True) or apply_workflow_yaml.get("on") or {}
    assert "push" not in on_triggers, (
        "terragrunt-apply.yml must NOT declare a live 'push' trigger while the lane is "
        "disabled -- it deploys to PROD and needs AWS credentials/OIDC."
    )
    assert "#   branches: [main]" in apply_workflow_text, (
        "terragrunt-apply.yml must preserve its push trigger (scoped to branch 'main') in "
        "commented-out form (AC-1, E7-F2-S1-T1) so re-enabling it is a pure uncomment."
    )
