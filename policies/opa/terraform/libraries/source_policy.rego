...
# True when the source literal is a monorepo git URL (example-org/telemetry-platform or caylent-solutions/terraform-modules).
# This is the single authoritative definition of the two-host allowlist.
is_monorepo_source(content) if {
	regex.match(`source\s*=\s*"git::https://github\.com/(caylent-solutions/terraform-modules|example-org/telemetry-platform)\.git//`, content)
}

# True when a monorepo source has a pinned /v<semver> ref.
has_ref_parameter(content) if {
	regex.match(`source\s*=\s*"git::https://github\.com/(caylent-solutions/terraform-modules|example-org/telemetry-platform)\.git//[^?]+\?ref=.+/v[0-9]+\.[0-9]+\.[0-9]+"`, content)
}
...
violation contains result if {
	...
	result := {
		...
		"resolution": "Use module sources from git::https://github.com/caylent-solutions/terraform-modules.git or git::https://github.com/example-org/telemetry-platform.git only",
	}
}
